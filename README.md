...
**Current Status**: **v0.1.17 - Offline-First Complete!** 500+ tests passing. SQLite event store, offline resilience, projection sync, embedded MCP, Tauri desktop integration, and lazy DI -- all with zero network dependencies!
...
[dependencies]
allframe = "0.1.17"       # Core framework
allframe-mcp = "0.1.17"   # MCP server - separate crate for zero bloat
...
AllFrame v0.1.17 is the first Rust web framework purpose-built for the way LLMs actually consume and produce APIs. Here's why this release matters if you're building LLM-powered applications:
...
LLMs speak [Model Context Protocol](https://modelcontextprotocol.io). AllFrame handlers automatically become MCP tools -- no glue code. New in v0.1.17: the embedded MCP server runs without a network port, so a desktop app (Tauri, Electron) can dispatch tool calls in-process:
...
```toml
[dependencies]
allframe = { version = "0.1.17", features = ["di", "openapi"] }
```
...
### Offline-First Features (✅ NEW in v0.1.17)
...
**Minimal REST API:**
```toml
allframe = { version = "0.1.17", default-features = false, features = ["router"] }
```

**Production GraphQL API:**
```toml
allframe = { version = "0.1.17", features = ["router-graphql"] }
```

**Multi-Protocol Gateway:**
```toml
allframe = { version = "0.1.17", features = ["router-full"] }
```
